...
const EnquiryForm = () => {
  ...
  try {
    const response = await fetch("http://localhost:5000/api/general-enquiry", {
      method: "POST",
...
<div>
  ...
  {errors.wish && (
    <p className="text-red-500 text-xs mt-1">{errors.wish}</p>
        ...
        {/* Meeting Type - Radio Buttons (Listed Vertically) */}
        <div>
  ...
  onChange={handleChange}
  required
  className="appearance-none w-4 h-4 border border-gray-400 rounded-full mr-2 cursor-pointer
             ...
             before:rounded-full before:bg-black before:opacity-0 checked:before:opacity-100
